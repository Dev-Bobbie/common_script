...
class QiushiSpider(object):
    ...
    def run(self):
        ...
        tasks = []
        get_url_list_task = threading.Thread(target=self.get_url_list)
        tasks.append(get_url_list_task)

        for i in range(5):
            parse_url_task = threading.Thread(target=self.parse_url)
            tasks.append(parse_url_task)

        for i in range(3):
            parse_html_task = threading.Thread(target=self.parse_html)
            tasks.append(parse_html_task)

        for i in range(2):
            save_item_task = threading.Thread(target=self.save_item)
